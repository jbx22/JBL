/**
 * Chat route helpers — Supabase-backed.
 *
 * Replaces the old Drizzle implementations with real Supabase REST queries.
 */
...
export async function canAccessProject(
  ...
  const shared = Array.isArray(project.shared_with)
    ? project.shared_with
    : [];
  return shared
    .map(String)
    .map((email) => email.toLowerCase())
    .includes(authUser.userEmail);
}
...
export async function getAccessibleChat(
  ...
  if (
    chat.project_id &&
    (await canAccessProject(chat.project_id, authUser))
  )
    return chat;
  return null;
}
...
export async function createChat(projectId?: string | null) {
  ...
  if (
    resolvedProjectId &&
    !(await canAccessProject(resolvedProjectId, authUser))
  ) {
    return NextResponse.json(
  ...
  if (error || !chat) {
    return errorResponse(error, "createChat");
  }
...
export async function streamAssistantChat(
  ...
  if (chatId) {
    ...
    if (
      requestedProjectId &&
      requestedProjectId !== existing.project_id
    ) {
      return NextResponse.json(
    ...
    if (createErr || !created) {
      return errorResponse(createErr, "streamAssistantChat create");
    }
    chatId = created.id;
    chatTitle = created.title;
  }

  // Increment message credits
  ...
  const currentCredits = currentProfile?.message_credits_used ?? 0;
  await supabase
    .from("user_profiles")
    .update({
      message_credits_used: currentCredits + 1,
...
export async function generateChatTitle(chatId: string, message: string) {
  try {
    ...
    return errorResponse(
      err,
      "POST /api/chat/[chatId]/generate-title error:"
    );
  }
}
...
export async function renameChat(chatId: string, title: string) {
  ...
  if (error) {
    return errorResponse(error, "renameChat");
  }
